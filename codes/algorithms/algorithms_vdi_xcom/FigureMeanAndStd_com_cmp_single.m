function FigureMeanAndStd_com_cmp_single(cellSegs, fig_options)
% 基于分段平均趋势作图（单组单子图）
    ...
    for i = 1:length(str_paras)
        ...
        [mean_curves{i}, std_curves{i}, time_curves{i}, time_stages{i}] = ...
            GetAverageCurves_ComCmp_Single(cellSegs, str_para);
    end
    ...
    FigureMeanAndStd_Color_Single(time_curves, mean_curves, std_curves, time_stages, ...
        str_paras, idxParas, fig_options);
end

function [mean_curve, std_curve, time_curve, time_stage] = GetAverageCurves_ComCmp_Single(cellSegs, str_para)
    mean_curve = [];
    std_curve = [];
    time_curve = [0];
    time_stage = [0];
    ...
    for idxStage = 1:3
        [datas_interp, time_mean] = GetAverageCurve_Segment_ComCmp(cellSegs, str_para, idxStage);
...
function FigureMeanAndStd_Color_Single(time_curves, mean_curves, std_curves, time_stages, ~, idxParas, fig_options)
    ...
    axes('Position',[0.12 0.2 0.78 0.7]);
    grid on;
    box on;
    ...
    for k = 1:length(idxParas)
        ...
        y_min(end+1) = min(signal_mean - signal_std);
        y_max(end+1) = max(signal_mean + signal_std);
    end

    y_min = min(y_min);
    y_max = max(y_max);
    ylim([y_min - 0.05 * range([y_min, y_max]), y_max + 0.05 * range([y_min, y_max])]);

    xlabel('Time / s', 'FontSize', font_size, 'FontName', font_name);
    ylabel(y_name, 'FontSize', font_size, 'FontName', font_name);
    xlim([0 2.5]);
%     title(title_name, 'FontName', font_name);

    legend(h_mean, legend_names, 'Location','eastoutside', ...
        'FontSize', font_size, 'FontName', font_name, 'Orientation', 'vertical', 'Box', 'off');
