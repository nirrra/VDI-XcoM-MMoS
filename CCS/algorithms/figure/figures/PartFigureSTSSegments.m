...
p5 = plot(times.union,ScaleData(stream.PELVIS.z,jointAngles.ankleAngleL),'Color', colors(3,:), 'LineWidth', 1.5, 'DisplayName', 'Pelvis Height');
plot(times.union,ScaleData(stream.HIP_LEFT.z,jointAngles.ankleAngleL),'Color', colors(3,:), 'LineWidth', 1);
plot(times.union,ScaleData(stream.HIP_RIGHT.z,jointAngles.ankleAngleL),'Color', colors(3,:), 'LineWidth', 1);
% p6 = plot(times.buttock,ScaleData(sum(sum(pressureButtock2D,2),3)./100,jointAngles.ankleAngleL),'Color', colors(4,:), 'LineWidth', 1.5, 'DisplayName', 'Buttock Pressure');
...
xline(sts_segments.time_p2, '--', 'Color', line_colors(6,:), 'LineWidth', 1, 'Alpha', 0.7);
xline(sts_segments.time_p3, '--', 'Color', line_colors(6,:), 'LineWidth', 1, 'Alpha', 0.7);
xline(sts_segments.time_end, '--', 'Color', line_colors(5,:), 'LineWidth', 1, 'Alpha', 0.7);

aux = ScaleData(stream.HIP_LEFT.z,jointAngles.ankleAngleL);
...
plot(times.union(sts_segments.idx_p2),jointAngles.ankleAngleL(sts_segments.idx_p2),'Color', colors(1,:),...
    'LineStyle', 'none', 'Marker','o', 'MarkerFaceColor', colors(1,:), 'MarkerSize', 8);
plot(times.union(sts_segments.idx_p3),jointAngles.hipFlexionL(sts_segments.idx_p3),'Color', colors(2,:),...
    'LineStyle', 'none', 'Marker','o', 'MarkerFaceColor', colors(2,:), 'MarkerSize', 8);

hold off; 
legend([p1, p2, p3, p4, p5], 'Location', 'eastoutside', 'FontSize', 9);
