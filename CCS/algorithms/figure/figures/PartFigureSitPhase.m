...
plot(times.buttock,sum(sum(pressureButtock2D,2),3)./10000,'Color', colors(1,:), 'LineWidth', 2.5);
plot(times.union,stream.PELVIS.z,'Color', colors(2,:), 'LineWidth', 2.5);
...
set(gca, 'FontSize', 10, 'XColor', [.3 .3 .3], 'YColor', [.3 .3 .3]);
histogram(stream.PELVIS.z(idx_segment_both),'EdgeColor',colors(1,:),'FaceColor',colors(2,:));
title('静坐段骨盆高度分布');
sgtitle('图.4 臀底无接触段');