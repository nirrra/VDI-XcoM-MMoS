...
for i = 1:length(hip_muscles)
    if isfield(muscle_activations, hip_muscles{i})
        p = plot(times.union, muscle_activations.(hip_muscles{i}), 'Color', colors(i,:), 'LineWidth', 2, 'DisplayName', hip_muscle_names{i});
...
title('Hip-related Muscle Activations (Left)', 'FontWeight', 'normal', 'FontSize', 11, 'Color', [.2 .2 .2]);
...
for i = 1:length(knee_muscles)
    if isfield(muscle_activations, knee_muscles{i})
        p = plot(times.union, muscle_activations.(knee_muscles{i}), 'Color', colors(i,:), 'LineWidth', 2, 'DisplayName', knee_muscle_names{i});
        plot_handles = [plot_handles, p];
    end
end
...
title('Knee-related Muscle Activations (Left)', 'FontWeight', 'normal', 'FontSize', 11, 'Color', [.2 .2 .2]);
xlabel('time / s'); ylabel('Activation');
ylim([0 1]);
...
for i = 1:length(ankle_muscles)
    if isfield(muscle_activations, ankle_muscles{i})
        p = plot(times.union, muscle_activations.(ankle_muscles{i}), 'Color', colors(i,:), 'LineWidth', 2, 'DisplayName', ankle_muscle_names{i});
        plot_handles = [plot_handles, p];
    end
end
...
title('Ankle-related Muscle Activations (Left)', 'FontWeight', 'normal', 'FontSize', 11, 'Color', [.2 .2 .2]);
xlabel('time / s'); ylabel('Activation');
ylim([0 1]);
...
for i = 1:length(lumbar_muscles)
    if isfield(muscle_activations, lumbar_muscles{i})
        p = plot(times.union, muscle_activations.(lumbar_muscles{i}), 'Color', colors(i,:), 'LineWidth', 2, 'DisplayName', lumbar_muscle_names{i});
        plot_handles = [plot_handles, p];
    end
end
...
title('Lumbar-related Muscle Activations (Left)', 'FontWeight', 'normal', 'FontSize', 11, 'Color', [.2 .2 .2]);
xlabel('time / s'); ylabel('Activation');
ylim([0 1]);

% 添加总标题
sgtitle('Muscle Activations by Joint', ...
...
print(f, ['./outputs/images muscle activations/',filename,'_muscle_activations.png'],'-dpng','-r300'); 