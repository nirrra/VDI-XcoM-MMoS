%% 计算对CoM的转动惯量
function I = CalRotaryInertia(gender,weight,stream,posCOMSegments,flag_move_to_origin) 

if nargin < 5
    flag_move_to_origin = true; % 是否平移到原点（踝关节）
end
...
for idxFrame = 1:length(stream.wtime)
    ...
    for i = 1:length(names)-1
        ...
        if flag_move_to_origin
            I(idxFrame,1) = I(idxFrame,1)+I1.(name)+I2.(nameLR);
        else
            I(idxFrame,1) = I(idxFrame,1)+I1.(name);
        end
    end
