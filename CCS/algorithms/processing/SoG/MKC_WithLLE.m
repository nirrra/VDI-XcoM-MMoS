function [value] = MKC_WithLLE(sog1,sog2,obj,K,lambda)
    % 体段权重+LLE项，公式14
    % 输入：
    %   sog1：点云的SoG
    ...
    % LLE项
    % 获取LLE权值矩阵
    [weights_LLE,neighborhood] = LLEWeight(sog2(:,1:3)',K);
    lle_sum = 0;
    for i = 1:size(sog2,1)
        gaussion1 = sog2(i,1:3);
        item = gaussion1;
        for j = 1:K
            neighbor = neighborhood(j,i); % 最近邻对应的点编号
            gaussion2 = sog2(neighbor,1:3);
            w_lle = weights_LLE(j,i);
            item = item-w_lle*gaussion2;
        end
        item = norm(item).^2;
        lle_sum = lle_sum+item;
    end
