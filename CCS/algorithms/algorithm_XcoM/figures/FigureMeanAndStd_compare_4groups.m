function FigureMeanAndStd_compare_4groups(cellSegs_patient_1, cellSegs_patient_2, cellSegs_patient_3, cellSegs_control, str_paras, colors, fig_options)
...
curves_control = curves_patient_1;
...
if nargin<7
    ...
    group_names = {'Patient Group 1','Patient Group 2','Patient Group 3','Control'};
    out_dir = './outputs/images_compare_4groups/';
    show_p2 = false;
    plot_bw = true;
else
...
time_p2(4) = GetAverageTime_p2(cellSegs_control);
...
% 计算四个组所有参数的最大时间点
curves_all = {curves_patient_1, curves_patient_2, curves_patient_3, curves_control};
max_time_1 = -inf;
max_time_2 = -inf;
max_time_3 = -inf;
max_time_4 = -inf;
for idxPara = 1:length(str_paras)
    ...
    t4 = curves_control.time_curves{idxPara};
    if ~isempty(t1)
    ...
    if ~isempty(t4)
        max_time_4 = max(max_time_4, max(t4));
    end
end
x_min = -0.2;
x_max = max([max_time_1, max_time_2, max_time_3, max_time_4]) + 0.2;

% 2x2 subplot arrangement
subplot_positions = [1, 2, 3, 4]; % 2x2 grid

for group = 1:4
    subplot(2,2,subplot_positions(group));
...
if plot_bw
    ...
    for group = 1:4
        ...
        legend(legend_handles, legend_names, 'Location','southoutside', ...
            'FontSize', 10, 'Interpreter', 'latex', 'Orientation', 'horizontal', 'Box', 'on');
    end
else
    % 彩色模式：使用原图例
    for group = 1:4
