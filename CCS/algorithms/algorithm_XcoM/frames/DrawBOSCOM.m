function [] = DrawBOSCOM(times,pressurePlantar2DInter,pressureButtock2DInter,...
    com,xcom,bos,bos_plantar,copPlantar,colors,transform_plantar2kinect,filename,flag_save,idxFrames)
...
if flag_frame
    ...
    if flag_save
        print(f, ['./outputs/images xcom bos/',filename,'_',num2str(idxFrame),'frame_BOS_COM.png'],'-dpng','-r300');
    end

else
    if flag_save
        outputVideo = VideoWriter(['./outputs/videos xcom bos/', filename, '_BOS_COM.mp4'], 'MPEG-4');
