function [ground_couple_moment_global_Left,ground_couple_moment_global_Right,moment_couple_global_new,...
    moment_com_global_new,moment_gravity_global_human_new] = Ground_Moment_WithPelvis(mass,sex,kinectstream,...
...
% male
m1 = 15.96; m2 = 16.33;
l1=170.7;l2=215.5;I1 = zeros(3,3);I2 = zeros(3,3);
I1(1,1)=(l1*0.454)^2;I1(2,2)=(l1*0.716)^2;I1(3,3)=(l1*0.659)^2;I1 = I1.*m1;
I2(1,1)=(l2*0.383)^2;I2(2,2)=(l2*0.482)^2;I2(3,3)=(l2*0.468)^2;I2 = I2.*m2;
d1 = [0;0;l1*0.2999-(l1+l2)*0.4161]; d2 = [0;0;(l1+l2*0.4502)-(l1+l2)*0.4161];
I1_d = I1+m1.*(sum(d1.^2,'all').*eye(3)-d1*d1');
I2_d = I2+m2.*(sum(d2.^2,'all').*eye(3)-d2*d2');
I_total = ((I1_d+I2_d)./((m1+m2).*(l1+l2).^2)).^0.5;
% female
m1 = 15.45; m2 = 14.65;
l1=142.5;l2=205.5;I1 = zeros(3,3);I2 = zeros(3,3);
I1(1,1)=(l1*0.502)^2;I1(2,2)=(l1*0.746)^2;I1(3,3)=(l1*0.718)^2;I1 = I1.*m1;
I2(1,1)=(l2*0.354)^2;I2(2,2)=(l2*0.433)^2;I2(3,3)=(l2*0.415)^2;I2 = I2.*m2;
d1 = [0;0;l1*0.2077-(l1+l2)*0.3727]; d2 = [0;0;(l1+l2*0.4512)-(l1+l2)*0.3727];
I1_d = I1+m1.*(sum(d1.^2,'all').*eye(3)-d1*d1');
I2_d = I2+m2.*(sum(d2.^2,'all').*eye(3)-d2*d2');
I_total = ((I1_d+I2_d)./((m1+m2).*(l1+l2).^2)).^0.5;
...
for i=1:len_Alfa
    ...
    moment_couple_global.Pelvis{i} = (r_all_segments.Pelvis{i}) * ((mass * M.Pelvis * Moment_of_Inertia.Pelvis) * ...
        [segments_Alpha_L.Pelvis{i}(3,2);segments_Alpha_L.Pelvis{i}(1,3);segments_Alpha_L.Pelvis{i}(2,1)]...
    ...
    % 所有体段力偶矩之和
    moment_couple_global.human{i} = moment_couple_global.HeadNeck{i} + moment_couple_global.Trunk{i} + moment_couple_global.Pelvis{i} + moment_couple_global.Upperarm_Left{i}...
...
for i=1:len_Alfa
    ...
    moment_com_global.Pelvis{i} = mass * M.Trunk *cross( [segments_com_position.Pelvis.x(i),segments_com_position.Pelvis.y(i),segments_com_position.Pelvis.z(i)],...
        [segments_com_acceleration.Pelvis.x(i),segments_com_acceleration.Pelvis.y(i),segments_com_acceleration.Pelvis.z(i)])';
    ...
    % 所有体段质心相对全局原点力矩之和
    moment_com_global.human{i} = moment_com_global.HeadNeck{i} + moment_com_global.Trunk{i} + moment_com_global.Pelvis{i} + moment_com_global.Upperarm_Left{i}...
...
for i=1:len_Alfa    
    ...
    moment_gravity_global_human{i} = cross([segments_com_position.HeadNeck.x(i),segments_com_position.HeadNeck.y(i),segments_com_position.HeadNeck.z(i)],[0,0,-mass * M.HeadNeck*9.8])'...
        ...
        +cross([segments_com_position.Pelvis.x(i),segments_com_position.Pelvis.y(i),segments_com_position.Pelvis.z(i)],[0,0,-mass * M.Pelvis*9.8])'...
