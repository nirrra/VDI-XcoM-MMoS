...
mu_s = 0.5;                  % 坐面等效摩擦系数上界，用于 BoS_adm/BoS_rob 的允许平移半宽
...
% BoS_nom: 名义有效支撑域，载荷加权 Minkowski 和，不含未测切向力偏移
bos_nom = struct();
bos_nom.x = cell(length(times.union),1);
bos_nom.y = cell(length(times.union),1);

% BoS_adm: 可达有效支撑域，bos_nom ⊕ [-Δs, Δs] in AP（扩展AP边界）
bos_adm = struct();
bos_adm.x = cell(length(times.union),1);
bos_adm.y = cell(length(times.union),1);

% BoS_rob: 鲁棒有效支撑域，bos_nom ⊖ [-Δs, Δs] in AP（收缩AP边界）
bos_rob = struct();
bos_rob.x = cell(length(times.union),1);
bos_rob.y = cell(length(times.union),1);
...
[bos_x_p_left, bos_y_p_left, mask_p_left] = CalBoS_new(imgPlantar_left, point_th_ratio, erode_size, upsample_factor, flag_show_fig);
[bos_x_p_right, bos_y_p_right, mask_p_right] = CalBoS_new(imgPlantar_right, point_th_ratio, erode_size, upsample_factor, flag_show_fig);
[bos_x_b, bos_y_b, mask_b] = CalBoS_new(imgButtock, point_th_ratio, erode_size, upsample_factor, flag_show_fig);
...
for idxFrame = 1:length(times.union)
    ...
    bos_x_p_left = bos_x_p_left + transform_plantar2kinect(1);
    bos_y_p_left = -bos_y_p_left + transform_plantar2kinect(2);
    bos_x_p_right = bos_x_p_right + transform_plantar2kinect(1);
    bos_y_p_right = -bos_y_p_right + transform_plantar2kinect(2);
    bos_x_p = bos_x_p + transform_plantar2kinect(1);
    bos_y_p = -bos_y_p + transform_plantar2kinect(2);
    bos_x_b = bos_x_b + transform_buttock2kinect(1);
    bos_y_b = -bos_y_b + transform_buttock2kinect(2);
    ...
    hasButtock = Fz_buttock_f(idxFrame) >= 50;
    ...
    elseif hasPlantar && ~hasButtock
        % 仅足底接触：bos_new = bos_nom = 足底 BoS（无臀底，delta_s=0）
        ...
        bos_nom.x{idxFrame} = bos_new.x{idxFrame};
        bos_nom.y{idxFrame} = bos_new.y{idxFrame};
        % delta_s = 0，bos_adm = bos_rob = bos_nom（在循环末尾统一处理）

    elseif ~hasPlantar && hasButtock
        % 仅臀底接触：bos_new 含切向力平移，bos_nom 不含
        ...
        bos_nom.x{idxFrame} = bos_buttock.x{idxFrame};
        bos_nom.y{idxFrame} = bos_buttock.y{idxFrame};
        delta_s = mu_s * height_sit; % alpha_buttock = 1
        ...
        if (hasPlantar_left && size(p_left,1) < 3) || (hasPlantar_right && size(p_right,1) < 3) || size(p_b,1) < 3
            bos_new.x{idxFrame} = [];
            bos_new.y{idxFrame} = [];
            bos_nom.x{idxFrame} = [];
            bos_nom.y{idxFrame} = [];
            ...
            n_use = nSample;
            if hasPlantar_left,  n_use = min(n_use, size(p_left,1));  end
            if hasPlantar_right, n_use = min(n_use, size(p_right,1)); end
            ...
            if n_use < 3
                ...
                % bos_nom：载荷加权 Minkowski 和，不含切向力偏移
                p_nom_pts = alpha_left * p_left + alpha_right * p_right + alpha_buttock * p_b;
                k_nom = convhull(p_nom_pts(:,1), p_nom_pts(:,2));
                bos_nom.x{idxFrame} = p_nom_pts(k_nom,1);
                bos_nom.y{idxFrame} = p_nom_pts(k_nom,2);

                % Δs = μs * hs * (1-α)，其中 1-α = alpha_buttock
                delta_s = mu_s * height_sit * alpha_buttock;
    ...
    % BoS_adm：bos_nom ⊕ [-Δs, Δs] in AP（名义域沿 AP 方向扩展 Δs）
    if isempty(bos_nom.x{idxFrame}) || delta_s == 0
        bos_adm.x{idxFrame} = bos_nom.x{idxFrame};
        bos_adm.y{idxFrame} = bos_nom.y{idxFrame};
    else
        xn = bos_nom.x{idxFrame};
        yn = bos_nom.y{idxFrame};
        pts = [xn, yn + delta_s; xn, yn - delta_s];
        ka = convhull(pts(:,1), pts(:,2));
        bos_adm.x{idxFrame} = pts(ka,1);
        bos_adm.y{idxFrame} = pts(ka,2);
    end

    % BoS_rob：bos_nom ⊖ [-Δs, Δs] in AP（名义域沿 AP 方向收缩 Δs）
    % 即 bos_nom 与 AP 区间 [y_min+Δs, y_max-Δs] 的矩形带的交集
    if isempty(bos_nom.x{idxFrame}) || delta_s == 0
        bos_rob.x{idxFrame} = bos_nom.x{idxFrame};
        bos_rob.y{idxFrame} = bos_nom.y{idxFrame};
    elseif max(bos_nom.y{idxFrame}) - min(bos_nom.y{idxFrame}) <= 2 * delta_s
        bos_rob.x{idxFrame} = [];
        bos_rob.y{idxFrame} = [];
    else
        xn   = bos_nom.x{idxFrame};
        ...
        y_hi = max(yn) - delta_s;
        xbuf = max(abs(xn)) + 1;
        ps_nom  = polyshape(xn, yn, 'Simplify', false);
        rect    = polyshape([-xbuf, xbuf, xbuf, -xbuf], [y_lo, y_lo, y_hi, y_hi]);
        ps_rob  = intersect(ps_nom, rect);
        if ps_rob.NumRegions > 0
            [xr, yr] = boundary(ps_rob, 1);
            bos_rob.x{idxFrame} = xr;
            bos_rob.y{idxFrame} = yr;
        else
            bos_rob.x{idxFrame} = [];
            bos_rob.y{idxFrame} = [];
        end
...
bos_x_p_fixed = bos_x_p_fixed + transform_plantar2kinect(1);
bos_y_p_fixed = -bos_y_p_fixed + transform_plantar2kinect(2);
[bos_x_b_fixed, bos_y_b_fixed, mask_b_fixed] = CalBoS_new(imgButtock, point_th_ratio, erode_size, upsample_factor);
bos_x_b_fixed = bos_x_b_fixed + transform_buttock2kinect(1);
bos_y_b_fixed = -bos_y_b_fixed + transform_buttock2kinect(2);
...
for idx_frame = 1:length(times.union)
    ...
    hasButtock_fixed = Fz_buttock_f(idx_frame) >= 50;
    if ~hasButtock_fixed
