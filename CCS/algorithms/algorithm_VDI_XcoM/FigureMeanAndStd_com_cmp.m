function FigureMeanAndStd_com_cmp(cellSegs_control, cellSegs_patient, fig_options)
% Plot segmented mean +/- std curves for control and patient groups.
    ...
    draw_compare_figure(curves_control, curves_patient, idxParas, group_names, fig_options);
end
...
function draw_compare_figure(curves_control, curves_patient, idxParas, group_names, fig_options)
    ...
    stage_line_width = get_or_default(fig_options, 'stage_line_width', 1.8);
    ...
    curves_by_group = {curves_control, curves_patient};
    all_y_min = [];
    all_y_max = [];
    pos = [0.08, 0.25, 0.36, 0.60];
    w = 0.44;
    legend_handles = gobjects(0);

    for iGroup = 1:2
        axes_handles(iGroup) = axes('Parent', f, 'Position', [pos(1) + w * (iGroup - 1), pos(2), pos(3), pos(4)]);
        hold on;
        grid on;
        box on;
        ...
        group_curves = curves_by_group{iGroup};
        h_mean = gobjects(1, length(idxParas));

        for j = 1:length(group_curves.time_stage)
            xline(group_curves.time_stage(j), ':', 'LineWidth', stage_line_width, 'Color', [0.55 0.55 0.55]);
        end
        ...
        for k = 1:length(idxParas)
            idxPara = idxParas(k);
            time = group_curves.time_curve;
            signal_mean = group_curves.mean_curves{idxPara};
            signal_std = group_curves.std_curves{idxPara};
            if isempty(signal_mean)
                continue;
            end

            if flag_std && any(isfinite(signal_mean))
                valid_fill = isfinite(time) & isfinite(signal_mean) & isfinite(signal_std);
                if any(valid_fill)
                    fill([time(valid_fill), fliplr(time(valid_fill))], ...
                         [signal_mean(valid_fill) + signal_std(valid_fill), ...
                          fliplr(signal_mean(valid_fill) - signal_std(valid_fill))], ...
                         colors(k, :) * 0.6 + 0.4, 'EdgeColor', 'none', 'FaceAlpha', fill_alpha);
                end
            end

            style_idx = mod(k - 1, length(line_styles)) + 1;
            h_mean(k) = plot(time, signal_mean, 'LineWidth', line_width, ...
                'Color', colors(k, :), 'LineStyle', line_styles{style_idx});
            if iGroup == 2
                legend_handles(end+1) = h_mean(k); %#ok<AGROW>
            end

            [curve_y_min, curve_y_max] = get_curve_bounds(signal_mean, signal_std, flag_std);
            if ~isempty(curve_y_min)
                y_min_local(end+1) = curve_y_min; %#ok<AGROW>
                y_max_local(end+1) = curve_y_max; %#ok<AGROW>
            end
        end

        if ~isempty(y_min_local)
            all_y_min(end+1) = min(y_min_local); %#ok<AGROW>
            all_y_max(end+1) = max(y_max_local); %#ok<AGROW>
        end
        ...
        title(group_names{iGroup}, 'FontName', font_name);
    end

    if ~isempty(all_y_min)
        y_span = max(all_y_max) - min(all_y_min);
        if y_span <= 0
            y_span = max(abs(all_y_max));
        end
        y_lim = [min(all_y_min) - 0.05 * y_span, max(all_y_max) + 0.05 * y_span];
        for iGroup = 1:2
            ylim(axes_handles(iGroup), y_lim);
        end
    end

    if ~isempty(legend_handles)
        legend(axes_handles(2), legend_handles, legend_names, 'Location', 'southoutside', ...
            'Position', [pos(1) + 0.78 * w, pos(2) - 0.23, 0.1, 0.1], ...
...
function [colors, line_styles] = bw_line_palette(n)
    ...
    for i = 1:n
        ...
        colors(i, :) = base_colors(idx, :);
        line_styles{i} = base_styles{idx};
    end
end
