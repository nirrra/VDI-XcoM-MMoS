...
% 计算MoS：sum为最短有符号距离，front/back/left/right为方向距离
    ...
    if isstruct(com) && isstruct(bos)
        if isstruct(times) && isfield(times, 'union')
            n = min([numel(com.x), numel(com.y), numel(bos.x), numel(bos.y), numel(times.union)]);
        else
            n = min([numel(com.x), numel(com.y), numel(bos.x), numel(bos.y)]);
        end
