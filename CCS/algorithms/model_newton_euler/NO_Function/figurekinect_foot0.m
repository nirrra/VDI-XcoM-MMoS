...
for i = 1
    ...
    %足迹识别
    footprint = FootprintMerge(Pathstream, numpath);
    [label, left, right, n] = Mark(footprint);%坐标还是原始坐标，向下为x，向右为y
    % 第二次去除坏点，脚印之外的点
    ...
    %判断步行开始方向
     start_Proximal = 0; %默认从远kinect端行走，Proximal：近端的
    if(mean(support{n})<mean(support{1})) %从靠近kinect端开始行走
        start_Proximal = 1; %表示从靠近kinect端开始行走
        supporttemp = support;
        for k = 1:n
    ...
    for t = tstart:1:tstop
        %画足迹
        if ismember(t / 1000, Pathstream.time)
            
            if ishandle(handle_footprint)
                delete(handle_footprint);%把上一次的图像清掉
            end
            
            f = find(Pathstream.time == t / 1000);
            [X, Y] = img2xy(double(Pathstream.x{f, 1}), double(Pathstream.y{f, 1}), numpath);
            Z=zeros(length(X),1)+0.01;
            handle_footprint = plot3(X,Y,Z,'.k');
        end
        %画人体
        if ismember(t / 1000, Kinectstream.wtime)
            
            if ishandle(p)
                delete(p);
            end
            
            m = find(Kinectstream.wtime == t / 1000);
